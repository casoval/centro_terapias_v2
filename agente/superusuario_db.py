...
MODELO_SONNET = 'gemini-2.5-flash'
MODELO_OPUS   = 'gemini-2.5-flash'
...
def _elegir_modelo(mensaje: str) -> tuple[str, str]:
    ...
    if any(p in msg for p in PALABRAS_OPUS) or len(mensaje.split()) > 30:
        return MODELO_OPUS, 'Gemini'
    return MODELO_SONNET, 'Gemini'
...
class AgenteSuper(AgenteBase):
    ...
    def responder(self, telefono: str, mensaje: str, staff=None) -> str:
        try:
            ...
            max_tokens = 2000 if etiqueta == 'Gemini' else 800
            ...
            if prompt_base:
                ...
                fallback = PROMPT_OPUS_FALLBACK if etiqueta == 'Gemini' else PROMPT_SONNET_FALLBACK
